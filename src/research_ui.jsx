...
// ═══════════════════════════════════════════════════════════════
// KNOWLEDGE GRAPH DATA — All 9 papers with full metadata
// ═══════════════════════════════════════════════════════════════
const KG = {
...
function Tag({ children, color = "#00ffaa" }) {
  return (
    <span style={{
      ...
      background: `${color}12`, color, display: "inline-block", margin: "2px",
    }}>{children}</span>
  );
}
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0" }}>
      <div style={{
        ...
        background: done ? color : active ? `${color}25` : "rgba(255,255,255,0.04)",
        border: `2px solid ${active || done ? color : "rgba(255,255,255,0.08)"}`,
        display: "flex", alignItems: "center", justifyContent: "center",
        fontSize: 12, fontWeight: 700, color: done ? "#000" : active ? color : "rgba(255,255,255,0.3)",
        boxShadow: active ? `0 0 20px ${color}60` : done ? `0 0 12px ${color}40` : "none",
        transition: "all 0.4s ease",
        animation: active ? "pulseBorder 1.5s ease infinite" : "none",
...
function RetrievedCard({ result, index }) {
  ...
  return (
    <div style={{
      background: "rgba(255,255,255,0.02)", border: `1px solid ${c}30`,
...
const PIPELINE_STEPS = [
  ...
  { label: "Claude LLM", sub: "Generating answer...", color: "#00eeff" },
];
...
export default function App() {
  ...
  const typedAnswer = useTypewriter(streaming ? "" : answer, 4);
  ...
  const runPipeline = useCallback(async (q) => {
    ...
    addLog("[LLM] Calling claude-sonnet-4-20250514...", "#00eeff");
    setStreaming(true);
    setActiveTab("answer");
    ...
    addLog(`[LLM] Done: ~${outTokens} tokens | ${elapsed}s`, "#00ffaa");

    // Compute quick eval metrics
    ...
    setMetrics({
      retrieved: fused.length, vecHits: vecResults.length, kgHits: kgResults.length,
      promptTokens: Math.round(prompt.split(" ").length * 1.3),
      outTokens, latency: elapsed,
      kwCov: +kwCov.toFixed(3), secComp: +sections.toFixed(2),
      topPaper: fused[0]?.paper.id, topScore: fused[0]?.score,
    });
    ...
    <div style={{ minHeight: "100vh", background: "#02040a", color: "#e8edf5", fontFamily: "'Outfit', sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes pulseBorder { 0%,100%{box-shadow:0 0 20px var(--pc,#00ffaa)60} 50%{box-shadow:0 0 35px var(--pc,#00ffaa)90} }
        @keyframes bounce { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-4px)} }
        @keyframes slideIn { from{opacity:0;transform:translateY(16px)} to{opacity:1;transform:translateY(0)} }
        @keyframes blink { 0%,100%{opacity:1} 50%{opacity:0} }
        @keyframes scanAnim { from{transform:translateY(0)} to{transform:translateY(4px)} }
        @keyframes spin { from{transform:rotate(0deg)} to{transform:rotate(360deg)} }
        @keyframes fadeIn { from{opacity:0} to{opacity:1} }
        .glow-text { text-shadow: 0 0 20px rgba(0,255,170,0.5); }
        textarea:focus { outline: none; }
        input:focus { outline: none; }
        button:hover .btn-inner { background: rgba(0,255,170,0.12); }
        .suggestion:hover { border-color: rgba(0,255,170,0.4) !important; background: rgba(0,255,170,0.05) !important; }
        .tab-active { border-bottom: 2px solid #00ffaa !important; color: #e8edf5 !important; }
        .hist-item:hover { border-color: rgba(0,255,170,0.3) !important; }
        .answer-text strong { color: #00ffaa; }
        .answer-text h2, .answer-text h3 { font-family: 'Unbounded', sans-serif; }
      `}</style>
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(2,4,10,0.9)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(255,255,255,0.06)", padding: "0 28px", height: 52, display: "flex", alignItems: "center", gap: 16 }}>
        ...
        <div style={{ flex: 1, overflow: "hidden", fontSize: 9, fontFamily: "'JetBrains Mono', monospace", color: "#4a5568" }}>
          <div style={{ animation: "slideIn 0.5s ease", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
            ◈ 9 PAPERS · 34 AUTHORS · 801 TRIPLES · 207 RELATIONSHIPS · CLAUDE SONNET 4 READY
          </div>
        </div>
          ...
          <div style={{ animation: "fadeIn 0.3s ease" }}>
            ...
            <div style={{ textAlign: "center", marginBottom: 36 }}>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#00ffaa", letterSpacing: 3, border: "1px solid rgba(0,255,170,0.3)", background: "rgba(0,255,170,0.06)", padding: "4px 16px", borderRadius: 99, display: "inline-flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
                ...
                HYBRID RAG PIPELINE — MILESTONE 3
              </div>
              ...
              <p style={{ color: "#718096", fontSize: 14, fontWeight: 300, maxWidth: 480, margin: "0 auto" }}>
                Ask anything about text summarization research. The pipeline retrieves papers from the knowledge graph, builds context, and generates a synthesized answer via Claude.
              </p>
            </div>

            {/* Search Box */}
            <div style={{ maxWidth: 760, margin: "0 auto 32px", position: "relative" }}>
              <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(0,255,170,0.2)", borderRadius: 16, overflow: "hidden", boxShadow: "0 0 40px rgba(0,255,170,0.07)", transition: "border-color 0.3s, box-shadow 0.3s" }}>
                <textarea
                  value={query}
                  onChange={e => setQuery(e.target.value)}
                  onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSubmit(); } }}
                  placeholder="Ask about text summarization methods, NLP approaches, transformer models, evaluation metrics..."
                  rows={3}
                  style={{ width: "100%", background: "transparent", border: "none", padding: "18px 20px 10px", fontSize: 14, color: "#e8edf5", fontFamily: "'Outfit', sans-serif", resize: "none", lineHeight: 1.6 }}
                />
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 16px", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                  <span style={{ fontSize: 10, fontFamily: "'JetBrains Mono', monospace", color: "#4a5568" }}>KG: 9 papers · Enter to send</span>
                  <button onClick={() => handleSubmit()} disabled={!query.trim() || phase === "running"} style={{
                    background: phase === "running" ? "rgba(0,255,170,0.1)" : "linear-gradient(135deg, #00ffaa, #0080ff)",
                    border: "none", borderRadius: 8, padding: "9px 22px",
                    fontFamily: "'JetBrains Mono', monospace", fontSize: 11, fontWeight: 700,
                    color: phase === "running" ? "#00ffaa" : "#000", cursor: phase === "running" ? "default" : "pointer",
                    letterSpacing: 1, opacity: !query.trim() ? 0.4 : 1,
            ...
            <div style={{ maxWidth: 760, margin: "0 auto 40px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(230px, 1fr))", gap: 8 }}>
                {SUGGESTED.map((s, i) => (
                  <button key={i} className="suggestion" onClick={() => { setQuery(s); handleSubmit(s); }} style={{
                    background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)",
                    borderRadius: 8, padding: "10px 12px", textAlign: "left",
                    fontFamily: "'Outfit', sans-serif", fontSize: 12, color: "#718096",
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  ...
                  <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: 18 }}>
                    ...
                    {PIPELINE_STEPS.map((s, i) => (
                      <PipelineStep key={i} {...s} active={activeStep === i} done={doneSteps.includes(i)} index={i} />
                    ))}
                  </div>

                  {/* Metrics */}
                  {metrics && (
                    <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(0,255,170,0.15)", borderRadius: 14, padding: 18, animation: "slideIn 0.4s ease" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  ...
                  <div style={{ background: "rgba(6,9,18,0.9)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, overflow: "hidden" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 14px", background: "rgba(255,255,255,0.03)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                      ...
                      <span style={{ flex: 1, textAlign: "center", fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: "#4a5568", letterSpacing: 2, textTransform: "uppercase" }}>Pipeline Log</span>
                    </div>
                    <div style={{ padding: 14, maxHeight: 150, overflowY: "auto", fontFamily: "'JetBrains Mono', monospace", fontSize: 11, lineHeight: 1.8 }}>
                      {logLines.map((l, i) => <div key={i} style={{ color: l.color, animation: "fadeIn 0.2s ease" }}>{l.msg}</div>)}
                  ...
                  {/* Answer */}
                  {(answer || streaming) && (
                    <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(0,255,170,0.2)", borderRadius: 14, overflow: "hidden", flex: 1, animation: "slideIn 0.4s ease" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 18px", background: "rgba(0,255,170,0.04)", borderBottom: "1px solid rgba(0,255,170,0.12)" }}>
                        ...
                        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#00ffaa", letterSpacing: 1, textTransform: "uppercase" }}>
                          {streaming ? "Claude is generating..." : "Answer Ready"}
                        </span>
                        {!streaming && metrics && (
                          <span style={{ marginLeft: "auto", fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: "#4a5568" }}>
                            {metrics.outTokens} tokens · {metrics.latency}s
                          </span>
                        )}
                      </div>
                      <div ref={answerRef} className="answer-text" style={{ padding: "20px 22px", maxHeight: 560, overflowY: "auto", fontSize: 13, lineHeight: 1.8, color: "#c8d5e8", whiteSpace: "pre-wrap" }}>
                        {streaming ? (
                          <>
                            {answer}
                            <span style={{ display: "inline-block", width: 8, height: 14, background: "#00ffaa", animation: "blink 1s step-end infinite", verticalAlign: "middle", marginLeft: 2 }} />
                          </>
                        ) : typedAnswer}
                      </div>
                    </div>
                  ...
                  {phase === "done" && (
                    <button onClick={() => { setPhase("idle"); setQuery(""); setAnswer(""); setRetrieved([]); setMetrics(null); setLogLines([]); }} style={{ background: "transparent", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, padding: "11px 0", fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#718096", cursor: "pointer", letterSpacing: 1, textTransform: "uppercase", transition: "all 0.2s" }}>
                      ↺ New Query
                    </button>
                  )}
                </div>
          ...
          <div style={{ animation: "fadeIn 0.3s ease" }}>
            {answer ? (
              <div style={{ maxWidth: 860, margin: "0 auto" }}>
                <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(0,255,170,0.2)", borderRadius: 14, overflow: "hidden" }}>
                  <div style={{ padding: "16px 22px", background: "rgba(0,255,170,0.04)", borderBottom: "1px solid rgba(0,255,170,0.12)", display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 8, height: 8, borderRadius: "50%", background: streaming ? "#ffd60a" : "#00ffaa", animation: streaming ? "bounce 1s ease infinite" : "none" }} />
                    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: streaming ? "#ffd60a" : "#00ffaa", letterSpacing: 1 }}>
                      {streaming ? "GENERATING..." : "ANSWER COMPLETE"}
                    </span>
                    {query && <span style={{ marginLeft: "auto", fontSize: 11, color: "#4a5568", maxWidth: 300, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>"{query}"</span>}
                  </div>
                  <div ref={answerRef} style={{ padding: "24px 28px", maxHeight: "70vh", overflowY: "auto", fontSize: 14, lineHeight: 1.9, color: "#c8d5e8", whiteSpace: "pre-wrap" }}>
                    {streaming ? <>{answer}<span style={{ display: "inline-block", width: 9, height: 15, background: "#00ffaa", animation: "blink 1s step-end infinite", verticalAlign: "middle", marginLeft: 2 }} /></> : typedAnswer}
                  </div>
                </div>
              </div>
            ) : (
              <div style={{ textAlign: "center", padding: 80, color: "#4a5568", fontSize: 13 }}>
                <div style={{ fontSize: 40, marginBottom: 12 }}>⬡</div>
                <div>Run a query first to see the answer here.</div>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === "pipeline" && (
          <div style={{ animation: "fadeIn 0.3s ease" }}>
            <div style={{ marginBottom: 28 }}>
              ...
              <div style={{ color: "#718096", fontSize: 13, fontWeight: 300 }}>Hybrid RAG system — TF-IDF Vector Search × Knowledge Graph Traversal × RRF Fusion × Claude LLM</div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 24 }}>
              <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: 22 }}>
                ...
                {[
                  { label: "TF-IDF Vector Store", pct: 60, color: "#0080ff" },
                  { label: "KG Graph Traversal", pct: 40, color: "#aa44ff" },
                ].map(b => (
                  <div key={b.label} style={{ marginTop: 14 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#718096", marginBottom: 5 }}>
                      <span>{b.label}</span><span style={{ color: b.color, fontFamily: "'JetBrains Mono', monospace" }}>{b.pct}%</span>
                    </div>
                    <div style={{ height: 5, background: "rgba(255,255,255,0.05)", borderRadius: 99, overflow: "hidden" }}>
                      <div style={{ height: "100%", width: b.pct + "%", background: b.color, borderRadius: 99, boxShadow: `0 0 8px ${b.color}60`, transition: "width 1s" }} />
                    </div>
                  </div>
                ))}
              </div>
              <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: 22 }}>
                ...
                {[
                  {n:"COLLABORATED_WITH",c:54,col:"#00ffaa"},{n:"AFFILIATED_WITH",c:49,col:"#0080ff"},
                  {n:"HAS_KEYWORD",c:36,col:"#aa44ff"},{n:"AUTHORED",c:34,col:"#ff6b35"},
                  {n:"BELONGS_TO_DOMAIN",c:13,col:"#ffd60a"},{n:"PUBLISHED_IN",c:9,col:"#00eeff"},
                  {n:"CITES",c:9,col:"#00ffaa"},{n:"SHARES_TOPIC",c:2,col:"#ff3366"},
                ].map(r => (
                  <div key={r.n} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 9 }}>
                    ...
                    <div style={{ flex: 1, height: 4, background: "rgba(255,255,255,0.04)", borderRadius: 99, overflow: "hidden" }}>
                      <div style={{ height: "100%", width: (r.c/54*100)+"%" , background: r.col, borderRadius: 99, boxShadow: `0 0 6px ${r.col}60` }} />
                    </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 10 }}>
              {[["Papers","9","#00ffaa"],["Authors","34","#0080ff"],["KG Triples","801","#aa44ff"],["Relationships","207","#ff6b35"],["Institutions","10","#ffd60a"],["Journals","8","#00eeff"],["Keywords","34","#00ffaa"],["Domains","12","#ff3366"],["References","9","#aa44ff"]].map(([l,v,c]) => (
                <div key={l} style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, padding: "14px 12px", textAlign: "center" }}>
                  <div style={{ fontFamily: "'Unbounded', sans-serif", fontSize: 28, fontWeight: 700, color: c, letterSpacing: -2, textShadow: `0 0 12px ${c}60` }}>{v}</div>
        ...
        {activeTab === "papers" && (
          <div style={{ animation: "fadeIn 0.3s ease" }}>
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ color: "#718096", fontSize: 13, fontWeight: 300 }}>9 papers in the knowledge graph — click to query</div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 14 }}>
              {PAPERS.map((p, i) => {
                ...
                const c = colors[i];
                return (
                  <div key={p.id} onClick={() => { setQuery(`Summarize and analyze: ${p.title}`); setActiveTab("query"); handleSubmit(`Summarize and analyze: ${p.title}`); }} style={{
                    background: "rgba(255,255,255,0.02)", border: `1px solid rgba(255,255,255,0.07)`,
                    borderRadius: 12, padding: 18, cursor: "pointer", position: "relative", overflow: "hidden",
                    transition: "all 0.25s", animation: `slideIn 0.4s ease ${i*0.05}s both`,
                  }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = c; e.currentTarget.style.boxShadow = `0 12px 40px rgba(0,0,0,0.5), 0 0 0 1px ${c}30`; e.currentTarget.style.transform = "translateY(-4px)"; }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(255,255,255,0.07)"; e.currentTarget.style.boxShadow = "none"; e.currentTarget.style.transform = ""; }}>
                    <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: `linear-gradient(90deg, ${c}, transparent)`, opacity: 0.6 }} />
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                      ...
                      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: "#4a5568" }}>{p.year}</span>
                      {p.shares_topic.length > 0 && <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: "#ff6b35", border: "1px solid rgba(255,107,53,0.3)", padding: "1px 7px", borderRadius: 99, background: "rgba(255,107,53,0.07)" }}>LINKED</span>}
                      <span style={{ marginLeft: "auto", fontSize: 9, color: "#4a5568", fontFamily: "'JetBrains Mono', monospace" }}>Click to query →</span>
                    </div>
                    <div style={{ fontFamily: "'Unbounded', sans-serif", fontSize: 13, fontWeight: 600, lineHeight: 1.35, marginBottom: 8, color: "#e8edf5" }}>{p.title}</div>
        ...
        {activeTab === "history" && (
          <div style={{ animation: "fadeIn 0.3s ease" }}>
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ color: "#718096", fontSize: 13, fontWeight: 300 }}>{history.length} queries this session</div>
            </div>
            {history.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {history.map((h, i) => (
                  <div key={i} className="hist-item" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, overflow: "hidden", animation: `slideIn 0.3s ease ${i*0.05}s both`, cursor: "pointer", transition: "border-color 0.2s" }}
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 18px", background: "rgba(255,255,255,0.02)", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
